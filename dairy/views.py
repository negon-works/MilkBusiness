...
def _missed_delivery_notifications_count(today=None):
    ...
    default_start = max(start_date, today - timedelta(days=45))
...
def home(request):
    ...
    unread_notifications_count = _missed_delivery_notifications_count(today)
...
def delivery_dashboard(request):
    ...
    snapshot = _delivery_snapshot(today)
    start_date = date(2026, 4, 1)
    history_months = list(DailyEntry.objects.filter(date__gte=start_date).dates('date', 'month', order='DESC'))
    history_year_choices = sorted({d.year for d in history_months}, reverse=True)
    ...
    if history_months:
        ...
        month_numbers = [d.month for d in history_months if d.year == selected_history_year]
        if selected_history_month not in month_numbers:
            selected_history_month = month_numbers[0]
        history_month_choices = [(m, month_name(m)) for m in month_numbers]
    else:
...
def delivery_notifications(request):
    ...
    oldest_entry = DailyEntry.objects.filter(date__gte=start_date).order_by('date').values_list('date', flat=True).first()
    default_start = max(start_date, today - timedelta(days=45))
    if oldest_entry is None:
        oldest_entry = default_start
    else:
...
def delivery_date_history(request):
    ...
    rows = []
    for entry in entries:
        litre = entry.morning_litre if mode == 'morning' else entry.evening_litre
        status = entry.morning_status if mode == 'morning' else entry.evening_status
        rows.append({'entry': entry, 'customer': entry.customer, 'litre': litre, 'status': status})
...
def customer_list(request):
    ...
    if billing == 'pending':
        today = date.today()
        pending_ids = MonthlyBill.objects.filter(year=today.year, month=today.month, is_paid=False).values_list(
            'customer_id', flat=True
        )
        customers = customers.filter(id__in=pending_ids)
...
def delivery_mode_start(request, mode):
    selected_date = _delivery_date_from_request(request)
    checked, total = _shift_counts_for_date(selected_date, mode)
    if total > 0 and checked >= total:
        return redirect(f"{reverse('delivery_summary', kwargs={'mode': mode})}?date={selected_date.isoformat()}")
    return redirect(f"{reverse('delivery_mode', kwargs={'mode': mode})}?date={selected_date.isoformat()}&i=0")
...
def delivery_mode(request, mode):
    ...
    saved_litre = entry.morning_litre if mode == 'morning' else entry.evening_litre
...
def feed_sack_list(request):
    purchases = FeedSackPurchase.objects.all()
    return render(request, 'dairy/feed_sack_list.html', {'purchases': purchases})
...
def feed_sack_create(request):
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        return redirect('feed_sack_list')
